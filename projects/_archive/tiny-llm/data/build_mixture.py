...
def build(name: str, tok_tag: str, total: int, val_frac: float, seed: int):
    ...
    for s in srcs:
        ...
        if not meta_p.exists():
            raise SystemExit(
                f"missing cache for {s.key!r} at {meta_p} — "
    ...
    caches = {
        s.key: np.memmap(tokdir / s.key / "ids.bin", dtype=DTYPE, mode="r")
        for s in srcs
    }
    ...
    for r in plan_rows:
        ...
        print(
            f"   {r['key']:<24} w={r['renorm_weight']:.3f} -> "
...
def main():
    ...
    ap.add_argument(
        "--name", default=None, help="mix name (default tiny_<budget>_<tag>)"
    )
    ...
    if args.weights:
        ov = {
            p.split(":")[0].strip(): float(p.split(":")[1])
            for p in args.weights.split(",")
        }
        for s in S.SOURCES:
            s.weight = ov.get(s.key, 0.0)
        print(
            "weight override ->", {s.key: s.weight for s in S.SOURCES if s.weight > 0}
        )

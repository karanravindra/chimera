...
# --- GPU usage + memory via nvidia-smi (guarded; averaged/summed across GPUs) ---
gpu=""
vram=""
if command -v nvidia-smi >/dev/null 2>&1; then
  read -r gpu vram < <(nvidia-smi --query-gpu=utilization.gpu,memory.used,memory.total \
    --format=csv,noheader,nounits 2>/dev/null | awk -F',' '
      {u+=$1; used+=$2; tot+=$3; n++}
      END{if(n>0) printf "%d %d", (u+n/2)/n, (tot>0)?(100*used+tot/2)/tot:0}')
fi
